    ...
    def perform_planning(self):
        with torch.no_grad():
            if self.debug_dset_init:
                # bypassa il planner: valuta direttamente le azioni vere del dataset
                actions = self.gt_actions.to(self.device)
                action_len = np.full(self.n_evals, np.inf)
            else:
            ...
            logs, successes, _, _ = self.evaluator.eval_actions(
                actions, action_len, save_video=True, filename="output_final"
            )
...
def planning_main(cfg_dict):
    ...
    if model_cfg.env.name == "wall" or model_cfg.env.name == "deformable_env" or model_cfg.env.name == "pushcube":
