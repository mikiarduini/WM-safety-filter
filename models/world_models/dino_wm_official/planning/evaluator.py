...
class PlanEvaluator:  # evaluator for planning
    ...
    def eval_actions(
        ...
        if self.wm.decoder is not None:
            # ----------------------------------------------------------------
            # VQVAE decode in mini-batches: processing all n_evals episodes at
            # once causes OOM (conv_transpose2d needs ~7 GiB).  We decode
            # DECODE_CHUNK episodes at a time, accumulate on CPU, then cat.
            # ----------------------------------------------------------------
            DECODE_CHUNK = 5
            visual_chunks = []
            n_ep = next(iter(i_z_obses.values())).shape[0]
            with torch.no_grad():
                for start in range(0, n_ep, DECODE_CHUNK):
                    end = min(start + DECODE_CHUNK, n_ep)
                    z_chunk = {k: v[start:end] for k, v in i_z_obses.items()}
                    torch.cuda.empty_cache()
                    chunk_vis = self.wm.decode_obs(z_chunk)[0]["visual"].cpu()
                    visual_chunks.append(chunk_vis)
                    del z_chunk, chunk_vis
            torch.cuda.empty_cache()
            i_visuals = torch.cat(visual_chunks, dim=0).to(self.device)
            del visual_chunks
            # ----------------------------------------------------------------
