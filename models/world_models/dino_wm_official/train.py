...
# ---------------------------------------------------------------------------
# How often (in optimizer steps) to overwrite the single on-disk checkpoint.
# Only one file is ever kept: checkpoints/model_latest.pth
# ---------------------------------------------------------------------------
...
class Trainer:
    def __init__(self, cfg):
        ...
        self.accelerator = Accelerator(log_with="wandb", mixed_precision="fp16")
        log.info(
        ...
        # ------------------------------------------------------------------
        # NEW: global optimizer-step counter, persisted in the checkpoint so
        # that the correct cadence is preserved across resume runs.
        # ------------------------------------------------------------------
        self.global_step = 0
        ...
        self._keys_to_save = [
            "epoch",
            "global_step",  # NEW: needed to restore the step counter on resume
        ]
        self._keys_to_save += (
    ...
    # ------------------------------------------------------------------
    # MODIFIED: save_ckpt now writes *only* checkpoints/model_latest.pth,
    # overwriting it each time. It strips the weak references introduced
    # by Accelerate on Optimizers to prevent pickling errors.
    # ------------------------------------------------------------------
    def save_ckpt(self):
        self.accelerator.wait_for_everyone()
        if self.accelerator.is_main_process:
            ...
            for k in self._keys_to_save:
                ...
                # RISOLUZIONE ERRORE: Estraiamo l'optimizer di base per rimuovere le "weakref" aggiunte da Accelerate
                if hasattr(obj, "optimizer") and "accelerate" in str(type(obj).__module__):
                    ckpt[k] = obj.optimizer
                elif hasattr(obj, "module"):
                    ckpt[k] = self.accelerator.unwrap_model(obj)
                else:
                    ckpt[k] = obj
                    
            # Single file, always overwritten — no epoch-stamped copy.
            torch.save(ckpt, "checkpoints/model_latest.pth")
            log.info(
                f"Checkpoint saved → checkpoints/model_latest.pth "
    ...
    # ------------------------------------------------------------------
    # MODIFIED: checkpoints are no longer saved here at epoch boundaries.
    # save_ckpt() is called from train() every SAVE_EVERY_STEPS steps.
    # Planning eval jobs are still triggered at epoch boundaries using the
    # always-up-to-date checkpoints/model_latest.pth.
    # ------------------------------------------------------------------
    def run(self):
        if self.accelerator.is_main_process:
        ...
        for epoch in range(init_epoch, init_epoch + self.total_epochs):
            ...
            # Planning eval jobs are still triggered on the configured epoch
            # cadence; they consume checkpoints/model_latest.pth — the only
            # file that exists on disk.
            if self.epoch % self.cfg.training.save_every_x_epoch == 0:
                if self.cfg.plan_settings.plan_cfg_path is not None:
                    # ckpt_path is only exposed to the main process so that
                    # plan launching happens exactly once.
                    ckpt_path = (
                        os.path.join(os.getcwd(), "checkpoints/model_latest.pth")
                        if self.accelerator.is_main_process
                        else None
                    )
                    model_name = self.cfg["saved_folder"].split("outputs/")[-1]
    ...
    def train(self):
        for i, data in enumerate(
            ...
            # ------------------------------------------------------------------
            # NEW: increment the global step counter and trigger a checkpoint
            # save every SAVE_EVERY_STEPS steps.  All processes reach this
            # barrier together because the accelerator-prepared dataloader
            # guarantees uniform batch counts across ranks.
            # ------------------------------------------------------------------
            self.global_step += 1
            if self.global_step % SAVE_EVERY_STEPS == 0:
                log.info(
                    f"Step {self.global_step}: saving checkpoint "
                    f"(epoch {self.epoch}, batch {i})..."
                )
                self.save_ckpt()
            ...
            # Log su wandb ogni SAVE_EVERY_STEPS step, stesso trigger del checkpoint
            if self.global_step % SAVE_EVERY_STEPS == 0 and self.accelerator.is_main_process:
                step_log = {"train_loss": loss.item(), "global_step": self.global_step}
                step_log.update({f"train_{k}": v for k, v in loss_components.items()})
